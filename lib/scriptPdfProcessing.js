...
module.exports = async (inputs, renderCallbackAsync) => {
  const { pdfContent, logger, operations, pdfMeta, pdfPassword, pdfSign, outlines, request } = inputs
  ...
  try {
    ...
    const manipulator = PdfManipulator(pdfBuf, { pdfMeta, pdfPassword, pdfSign, outlines, hiddenPageFields: request.context.shared.pdfUtilsHiddenPageFields })
