...
module.exports = async (inputs, callback, done) => {
  ...
  const { pdfContent, operations, pdfMeta, pdfPassword, pdfSign, outlines } = inputs
  let logs = []
  ...
  try {
    ...
    const manipulator = PdfManipulator(pdfBuf, { pdfMeta, pdfPassword, pdfSign, outlines })
    const operationsToProcess = operations.filter(o => o.templateShortid || o.template)

    log('debug', `pdf-utils detected ${operationsToProcess.length} pdf operation(s) to process`)
    ...
    for (const operation of operationsToProcess) {
      ...
      log('debug', `pdf-utils running pdf operation ${operation.type}`)
    ...
    if (inputs.outlines) {
      log('debug', `pdf-utils adding pdf outlines`)
    }

    if (pdfMeta) {
      log('debug', `pdf-utils will add metadata information to pdf`)
    }

    if (pdfPassword) {
      log('debug', `pdf-utils will add password (encryption/privileges) to pdf`)
    }

    if (pdfSign) {
      log('debug', `pdf-utils will add digital signature to pdf`)
    }
