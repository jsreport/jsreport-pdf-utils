...
module.exports = (reporter, definition) => {
  reporter.documentStore.registerComplexType('PdfOperationType', {
    ...
    // used only with merge type
    mergeLayer: {type: 'Edm.String'},
    renderForEveryPage: {type: 'Edm.Boolean'}
  })

  reporter.documentStore.registerComplexType('PdfUtilsType', {
    // we store it outside operations array, but it is used only when a merge operation is present
    removeContentBackLayer: {type: 'Edm.Boolean'}
  })

  if (reporter.documentStore.model.entityTypes['TemplateType']) {
    reporter.documentStore.model.entityTypes['TemplateType'].pdfOperations = { type: 'Collection(jsreport.PdfOperationType)' }
    reporter.documentStore.model.entityTypes['TemplateType'].pdfUtils = { type: 'Collection(jsreport.PdfUtilsType)' }
  }
  ...
  reporter.afterRenderListeners.insert(0, 'pdf-utils', async (req, res) => {
    ...
    for (const operation of req.template.pdfOperations.filter(o => o.templateShortid)) {
      ...
      if (operation.type === 'merge') {
        ...
        await manipulator.merge(pagesBuffers, operation.mergeLayer,
          req.template.pdfUtils && boolOrUndefined(req.template.pdfUtils.removeContentBackLayer))
        continue
      }
    }
