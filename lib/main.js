...
module.exports = (reporter, definition) => {
  ...
  reporter.afterTemplatingEnginesExecutedListeners.add('pdf-utils', (req, res) => {
    if (!req.template.content.includes('data-pdf-utils-outline')) {
      // optimization, don't do parsing if there is not a single link enabled
      return
    }

    const $ = cheerio.load(req.template.content)
    const anchors = $('a[data-pdf-utils-outline]')

    req.context.pdfUtilsOutlines = anchors.map((i, a) => ({
      id: $(a).attr('href').substring(1),
      text: $(a).attr('data-pdf-utils-outline-text'),
      parent: $(a).attr('data-pdf-utils-outline-parent') || null
    })).get()
  })
