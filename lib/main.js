...
module.exports = (reporter, definition) => {
  ...
  if (reporter.documentStore.model.entityTypes['TemplateType']) {
    ...
    reporter.documentStore.model.entityTypes.TemplateType.pdfPassword = { type: 'jsreport.PdfPasswordType' }
  }
  ...
  reporter.afterRenderListeners.insert(0, 'pdf-utils', async (req, res) => {
    if (req.template.pdfPassword == null && 
      (!req.template.pdfOperations || req.template.pdfOperations.length === 0) && !req.context.pdfUtilsOutlines) {
      return
    }
