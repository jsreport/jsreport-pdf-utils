...
module.exports = (reporter, definition) => {
  ...
  if (reporter.compilation) {
    reporter.compilation.resourceInTemp('scriptPdfProcessing.js', path.join(__dirname, 'scriptPdfProcessing.js'))
  }
  ...
  reporter.afterRenderListeners.insert(0, 'pdf-utils', async (req, res) => {
    ...
    const result = await reporter.executeScript({
      ...
      execModulePath: reporter.execution ? reporter.execution.resourceTempPath('scriptPdfProcessing.js') : path.join(__dirname, 'scriptPdfProcessing.js'),
