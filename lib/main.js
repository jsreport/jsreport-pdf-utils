...
module.exports = (reporter, definition) => {
  ...
  reporter.beforeRenderListeners.insert({ after: 'data' }, 'pdf-utils', async (req, res) => {
    ...
    function pdfCreatePagesGroup (groupId) {
      ...
      // we use display: contents on the parent to make the element to not participate in flexbox layout
      // (making it not a flexbox child)
      const result = `<span style='display: contents;'><span style='opacity: 0.01;font-size:1.1px'>group@@@${value}@@@</span></span>`
    ...
    function pdfAddPageItem (item) {
      ...
      // we use display: contents on the parent to make the element to not participate in flexbox layout
      // (making it not a flexbox child)
      const result = `<span style='display: contents;'><span style='opacity: 0.01;font-size:1.1px'>item@@@${value}@@@</span></span>`
