...
module.exports = (reporter, definition) => {
  ...
  reporter.beforeRenderListeners.insert({ after: 'data' }, 'pdf-utils', async (req, res) => {
    ...
    function pdfFormField (el) {
      ...
      return `<span class='jsreport-pdf-utils-form-element jsreport-pdf-utils-hidden-element' style='font-family: Helvetica;display: inline-block;vertical-align:middle;text-transform:none;font-size:1.5px;width: ${el.width}px; height: ${el.height}px'>form@@@${value}@@@</span>`
    }
    ...
    function pdfCreatePagesGroup (groupId) {
      ...
      const result = `<span class='jsreport-pdf-utils-page-group jsreport-pdf-utils-hidden-element' style='font-family: Helvetica;position:absolute;text-transform: none;opacity: 0.01;font-size:1.5px'>group@@@${value}@@@</span>`
    ...
    function pdfAddPageItem (item) {
      ...
      // we use position: absolute to make the element to not participate in flexbox layout
      // (making it not a flexbox child)
      const result = `<span class='jsreport-pdf-utils-page-item jsreport-pdf-utils-hidden-element' style='font-family: Helvetica;position:absolute;text-transform: none;opacity: 0.01;font-size:1.5px'>item@@@${value}@@@</span>`
