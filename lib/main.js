...
module.exports = (reporter, definition) => {
  ...
  reporter.beforeRenderListeners.insert({ after: 'data' }, 'pdf-utils', async (req, res) => {
    ...
    function pdfFormElement (el) {
      ...
      if (el == null || el.type == null || el.name == null || el.width == null || el.height == null) {
        throw new Error('pdfFormElement requires name, type, width, height params ')
      }      

      if (!el.width.includes('px')) {
        throw new Error('pdfFormElement width should be in px')        
      }

      el.width = parseInt(el.width.substring(0, el.width.length - 2))

      if (!el.height.includes('px')) {
        throw new Error('pdfFormElement height should be in px')        
      }
      ...
      if (el.fontSize != null) {
        if (!el.fontSize.includes('px')) {
          throw new Error('pdfFormElement fontSize should be in px')        
        }
      ...
      if (el.type === 'combo' && el.items == null) {
        throw new Error('pdfFormElement with combo type needs requires items attribute')
      }
      ...
      if (el.fontFamily != null) {
        ...
        if (!stdFonts.includes(el.fontFamily)) {
          throw new Error('pdfFormElement supports only pdf base 14 fonts in fontFamily attribute.')
        }
      }
  ...
  reporter.afterRenderListeners.insert(0, 'pdf-utils', async (req, res) => {
    ...
    if (
      ...
      pdfPassword = req.template.pdfPassword.password
    }
