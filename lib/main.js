...
module.exports = (reporter, definition) => {
  ...
  reporter.afterTemplatingEnginesExecutedListeners.add('pdf-utils', (req, res) => {
    ...
    anchors.each(function (i, a)  {
      ...
      if (!href || href[0] !== '#') {
        throw reporter.createError('Invalid url passed to anchor href with data-pdf-utils-outline attribute.', {
          statusCode: 400
        })
      }

      const title = $(this).attr('data-pdf-utils-outline-title') || $(this).text().trim()

      if (!title) {
        throw reporter.createError('Invalid value passed to data-pdf-utils-outline-title.', {
          statusCode: 400
        })
      }

      const parent = $(this).attr('data-pdf-utils-outline-parent') || null      

      if (parent && !req.context.pdfUtilsOutlines.find(o => o.id == parent)) {
        throw reporter.createError(`Outline parent "${parent}" passed to pdata-pdf-utils-outline-parent was not found.`, {
          statusCode: 400
        })
      }
