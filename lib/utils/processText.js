...
async function processStream (doc, streamObject, { page, pages, pageIndex, cmapCache, formsProcessor, removeHiddenMarks, hiddenPageFields }) {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    if (line.endsWith('Tm')) {
      ...
      // the top margin of chrome settings changes possitions using cm instructions, here we adapt the position
      // the cm instructions changes current matrix
      // with top 1cm it looks like this
      // 3.1250002 0 0 3.1250002 0 116.666672 cm
      // based on spec, the 0 index respresents the scale for x, the 5 index represents the transform for x
      // translate: [ 1 0 0 1 x y ]
      // scale:     [ x 0 0 y 0 0 ]
      currentPosition[0] += currentMatrix[4] / currentMatrix[0]
      currentPosition[1] += currentMatrix[5] / currentMatrix[3]
    }

    if (line.endsWith('cm')) {
      const fragments = line.split(' ')
      currentMatrix = fragments.slice(0, -1).map(n => parseFloat(n))
    }
