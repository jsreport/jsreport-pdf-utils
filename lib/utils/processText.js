...
async function processStream (doc, streamObject, page, pages, pageIndex, cmapCache, formsProcessor) {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    if (line.endsWith('Do')) {
      ...
      await processStream(doc, xobjectContent, xobjectObject, pages, pageIndex, cmapCache, formsProcessor)
    }
    ...
    if (line.endsWith('Tj')) {
      ...
      for (const charSeq of chunkArray(hexes, 4)) {
        ...
        text += ch
        details.push({ lineIndex, charIndex, ch, position: currentPosition, font: currentFontRef })
  ...
  let indexes = getAllIndexes(text, ['group', 'item', 'form'])
  ...
  for (const index of indexes) {
    ...
    if (detailsToProcess.length > 0) {
      ...
      if (text.includes('form@@@')) {
        await formsProcessor(doc, detailsToProcess.map(d => d.ch).join(''), detailsToProcess[0].font, detailsToProcess[0].position, pages[pageIndex].object, pageIndex, height)
      }
    }

    // no we remove the text from stream
...
module.exports = async (doc, ext) => {
  ...
  try {
    for (const page of ext.pages.get('Kids')) {
      ...
      await processStream(doc, streamObject, pageObject, ext.pages.get('Kids'), pageIndex, new Map(), FormsProcessor(doc, ext))
      pageIndex++
    }
  } catch (e) {
    if (e.isFormError) {
