...
module.exports = class EmbeddedPdf {
  constructor (page) {
    const mediaBox = page.object.properties.get('MediaBox')
    this.page = page
    this.width = mediaBox[2]
    this.height = mediaBox[3]

    const contents = this.page.object.properties.get('Contents')
    this.xobjCount = Array.isArray(contents) ? contents.length : 1
  }

  async write (doc, xobjs) {
    const resources = this.page.object.properties.get('Resources')
    ...
    for (let i = 0; i < this.xobjCount; ++i) {
      ...
      let contents = this.page.object.properties.get('Contents')
